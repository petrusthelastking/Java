...
class User {
    ...
    for (int i = 0; i < bookingHistory.length; i++) {
        if (bookingHistory[i][0] != null) {
            ...
            String tanggal = String.format("%17s", bookingHistory[i][2]);
            String jenisGedung = String.format("%13s", bookingHistory[i][0]);
            
            // Handle nilai fasilitas yang mungkin null
            String fasilitas = (bookingHistory[i][6] != null) ? String.format("%14s", bookingHistory[i][6]) : bookingInfo[0] ;

            String opsiPemesanan = String.format("%16s", bookingHistory[i][1]);
